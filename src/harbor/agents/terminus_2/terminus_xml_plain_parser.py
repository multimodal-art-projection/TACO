...
class TerminusXMLPlainParser:
    ...
    VALID_COMPRESSION_FEEDBACK = {"need_full_output"}
    ...
    def _try_parse_response(self, response: str) -> ParseResult:
        ...
        compression_feedback = self._parse_compression_feedback(
            sections.get('compression_feedback', ''),
            warnings,
        )
        ...
        if not commands_content:
            if 'commands' in sections:
                ...
                return ParseResult(
                    [], is_complete, "",
                    "- " + "\n- ".join(warnings) if warnings else "",
                    analysis, plan, compression_feedback
                )
            else:
    ...
    def _extract_sections(self, content: str, warnings: List[str]) -> dict:
        ...
        section_patterns = {
            ...
            'compression_feedback': (
                r'<compression_feedback>(.*?)</compression_feedback>',
                r'<compression_feedback\s*/>',
                r'<compression_feedback></compression_feedback>',
            ),
            'task_complete': (
        ...
        expected_tags = set(
            self.required_sections + ['compression_feedback', 'task_complete']
        )
        unexpected = set(top_level_tags) - expected_tags
        for tag in unexpected:
            warnings.append(
                f"Unknown tag found: <{tag}>, expected "
                f"analysis/plan/commands/compression_feedback/task_complete"
            )
        
        # Check for multiple instances of same tag
        for section_name in self.required_sections + ['compression_feedback', 'task_complete']:
    ...
    def _parse_compression_feedback(
        self,
        feedback: str,
        warnings: List[str],
    ) -> str:
        """Extract and validate structured compression feedback."""
        normalized = feedback.strip()
        if not normalized:
            return ""
        if normalized not in self.VALID_COMPRESSION_FEEDBACK:
            warnings.append(
                "Field 'compression_feedback' must be 'need_full_output' when used"
            )
            return ""
        return normalized
