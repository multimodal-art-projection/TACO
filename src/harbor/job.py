...
class Job:
    ...
    async def run(self) -> JobResult:
        ...
        try:
            ...
            # Snapshot the self-evolving compression rule cache next to the job result.
            try:
                from pathlib import Path

                cache_path = Path.home() / ".harbor" / "compression_rules_cache.json"
                if cache_path.exists():
                    final_snapshot = (
                        self.job_dir / "compression_rules_cache_final.json"
                    )
                    shutil.copy2(cache_path, final_snapshot)
                    self._logger.info(
                        f"Compression rules cache snapshot saved to {final_snapshot}"
                    )
            except Exception as exc:
                self._logger.warning(
                    f"Failed to save compression rules cache snapshot: {exc}"
                )
